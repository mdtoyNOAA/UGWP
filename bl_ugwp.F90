...
!
   ...
   endif ! ( (ugwp_ms .eq. 1 .or. ugwp_bl .eq. 1) .and. (ls_taper(i) .gt. 1.E-02) )

enddo    ! i = its,ite

do i = its,ite
   dusfcg(i) = -inv_g * dusfc(i)
   dvsfcg(i) = -inv_g * dvsfc(i)
enddo
                                                  
if ( ugwp_diags ) then
!  Finalize dusfc and dvsfc diagnoses
   do i = its,ite
      dusfc_ls(i) = -inv_g * dusfc_ls(i)
      dvsfc_ls(i) = -inv_g * dvsfc_ls(i)
      dusfc_bl(i) = -inv_g * dusfc_bl(i)
      dvsfc_bl(i) = -inv_g * dvsfc_bl(i)
   enddo
endif
